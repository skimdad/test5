...
(function ($) {	
	...
	h = $("head")[append](style+".CRZ{table-layout:fixed;}.CRZ td,.CRZ th{"+pa+left+zi+pa+"right"+zi+"overflow:hidden}.CRC{height:0px;overflow-x:visible;overflow-y:-moz-scrollbars-none;"+position+":relative;}.CRG{margin-left:-5px;"+position+absolute+"z-index:5;}.CRG .CRZ{"+position+absolute+"background-color:red;filter:alpha(opacity=1);opacity:0;width:10px;height:100%;top:0px}.CRL{"+position+absolute+"width:1px}.CRD{ border-left:1px dotted black"+styleEnd);
	...
	function init(tb, options) {
		...
		//if($("td.OBGridFilterCell").hasClass('OBGridFilterCell')){	
	...
	$("tr[id*='bin-row-']").live('click',function(){
	...
	function syncCols(t, i, isOver) {
		...
		if (isOver) {
			...
			$("span[id*='content-id-']").live('click', function(){

				current_id = $(this).attr('id');

				$("#yes_no_dropdown"+current_id).css('width',w);

			});

			if($("#yes_no_dropdown"+current_id).hasClass('dropdown_div')){
	...
	function onGripDrag(e) {
		...
		col_width = t.g[i][position]()[left] - t.g[i-1][position]()[left];
		...
		$("span[id*='content-id-']").live('click', function(){				
			...
			$("#yes_no_dropdown"+row_id).css('width',col_width);

		});

		if($("#yes_no_dropdown"+row_id).hasClass('dropdown_div')){
