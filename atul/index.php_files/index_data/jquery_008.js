	...
	$.fn.columnFilters = function(settings) {
		...
		return this.each(function() {
			...
			function addClassToColumn(iColNum, sClassName) {
				...
				var ids_array = Array('bin-row-' , 'rows-bin-contents-' , 'row-subtable','rows-subtable')



				for( var w = 0 ; w<ids_array.length ; w++){



				$("tr[id*='"+ids_array[w]+"']", obj).each(
					function() {
						$('td', this).each(
							function(iCellCount) {
			...
			function runFilters(event) {	
				...
				$('input._filterText', obj).each(
					function(iColCount) {
						...
						var ids_array = Array('bin-row-' , 'rows-bin-contents-' , 'row-subtable')
						...
						$("tr[id*='"+ids_array[w]+"']", obj).each(
							function() {
								$('td',this).each(
									function(iCellCount) {
										if (iCellCount === iColCount) {
											var sVal = $(this).text().replace(/(\n)|(\r)/ig,'').replace(/\s\s/ig,' ').replace(/^\s/ig,'');								//alert(bMatch+'    '+filterPatt.test(sVal)+'    '+sVal);
								...
								if ($('td',this).length !== $('td._match',this).length) {
									...
									$(this).find("input[name^='csv_arr_next']").each(function(){
										var xyz = $(this).attr('name').split('csv_arr_next');
										$(this).attr('name','csv_arr'+xyz[1]);	
									});
									...
									$(this).parents().find('div.content').css('min-height',26);
			...
			var ids_array1 = Array('bin-row-' , 'rows-bin-contents-' , 'row-subtable')



			for( var w1 = 0 ; w1<ids_array1.length ; w1++){



			$("tr[id*='"+ids_array1[w1]+"']", obj).each(
				function(iColCount) {
					...
					$("input[id*='text_storage-bin-']").each(function(){
						$(this).addClass('_filterText');
						$(this).keyup(
							function() { 						
								clearTimeout(filter); 
								filter = setTimeout(runFilters, $("tr[id*='"+ids_array1[w1]+"']", obj).length*2);
								if (settings.underline) {
					...
					if (settings.excludeColumns && settings.excludeColumns.length) {
						for (var i=0; i < settings.excludeColumns.length; i++) {
							if (settings.excludeColumns[i] === iColCount) {
								//$("input[id*='text_storage-bin-']").css('display','none');
							}
						}
					}
